...
function toggleCard(id) {
  ...
  let notesA = JSON.parse(localStorage.getItem('notes')) || []; // Get notes from local storage

  let noteList = document.getElementById('note-list');
  
  // Add event listener to search bar
  searchBar.addEventListener('input', function() {
    let searchQuery = searchBar.value.toLowerCase(); // Get search query
    
    // Filter notes and update note list
    let filteredNotes = notesA.filter(function(note) {
      return note.title.toLowerCase().includes(searchQuery) || note.text.toLowerCase().includes(searchQuery);
    });
    
    updateNoteList(filteredNotes); // Update note list
  });
  
  // Update note list function
  function updateNoteList(notes) {
    notesDiv.innerHTML = ''; // Clear note list
    // Iterate through filtered notes and add them to note list
    notes.forEach(function(note) {
        
      let noteCard = document.createElement('div');
      noteCard.classList.add('note');
      noteCard.innerHTML = `<h2>  ${note.title === "" ? 'Note' : note.title}  </h2>  <p>  ${note.text}  </p>`;
